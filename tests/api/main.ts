...
test("AUTOPAGE-01", "Enable autoCreateDashboardPageOnFormCreate via restore endpoint", async () => {
  const res = await adminClient.post<Record<string, unknown>>(
    "/api/admin/config/backup?sections=admin&mode=append",
    { admin: { features: { autoCreateDashboardPageOnFormCreate: true } } },
  );
  eq("status", res.status, 200);
});
...
test("AUTOPAGE-03", "Create a fresh form → auto-page appended bound to its id", async () => {
  ...
  const pages = (backup.body.admin as Record<string, unknown>).pages as Array<Record<string, unknown>>;
  const autoPage = pages.find(p => p.autoGeneratedFor === createdAutoFormId);
  ok("auto-page exists for the new form", !!autoPage);
  ok("auto-page is a submissions_table page", Array.isArray(autoPage?.widgets) && (autoPage!.widgets as Array<Record<string, unknown>>).some(w => w.type === "submissions_table"));
});

test("AUTOPAGE-04", "Delete form → its auto-page is removed", async () => {
  ...
  const pages = (backup.body.admin as Record<string, unknown>).pages as Array<Record<string, unknown>>;
  const orphan = pages.find(p => p.autoGeneratedFor === createdAutoFormId);
  ok("auto-page deleted along with the form", !orphan);
  createdAutoFormId = "";
});
...
test("AUTOPAGE-05", "Disable the toggle — restore endpoint resets it", async () => {
  const res = await adminClient.post<Record<string, unknown>>(
    "/api/admin/config/backup?sections=admin&mode=append",
    { admin: { features: { autoCreateDashboardPageOnFormCreate: false } } },
  );
  eq("status", res.status, 200);
});
