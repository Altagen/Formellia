...
describe("mergeAdminViews", () => {
  ...
  it("append mode throws when an incoming page has no id", () => {
    expect(() => mergeAdminViews(current, [{ title: "x", slug: "x", widgets: [] } as unknown as AdminView], "append")).toThrow();
  });

  it("append into an empty config just adds the pages", () => {
    expect(mergeAdminViews([], [page("a"), page("b")], "append").map(p => p.id)).toEqual(["a", "b"]);
  });
});
