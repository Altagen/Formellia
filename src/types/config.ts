...
export interface AdminPage {
  ...
  /** Optional folder id (references AdminConfig.folders). */
  folderId?: string;
  /**
   * When set, this page was created automatically by `autoFormView` on
   * form creation and can be safely regenerated / removed by the same
   * lifecycle. The value is the `formInstanceId` it belongs to.
   */
  autoGeneratedFor?: string;
}

export interface AdminFeatures {
  /** Audit log (/admin/audit) — admin action history, default: false */
  auditLog?: boolean;
  /** When true, `autoFormView` creates a scoped dashboard on form creation. */
  autoCreateDashboardPageOnFormCreate?: boolean;
}

/**
 * Hierarchical folder used to group forms or views under /admin/forms and
 * /admin/views. Root items (no parentId) live at the top level; nested items
 * (parentId set) appear when the user drills into their parent folder.
 */
...
export interface AdminConfig {
  ...
  /** When true, an "all submissions" admin view is auto-created on form creation
   *  and removed on form deletion. The view stays editable like any other. */
  autoGenerateView?: boolean;
  /** Folders that group forms or views under /admin/forms and /admin/views.
   *  Items live in a folder when their `folderId` matches a folder id. */
  folders?: AdminFolder[];
  /** Feature toggles pre-applied to every new form (UI creation or YAML restore). */
  defaultFormFeatures?: {
    formVersioning?: boolean;
    blockDisposableEmails?: boolean;
  };
  /** Preset reasons offered when excluding submissions from a DataPool. */
  exclusionReasons?: string[];
  /** Retention policy for the admin audit log (admin_events table). Purge runs
   *  as an on-demand or scheduled job. `keep_all` disables purging. */
  auditRetention?: AuditRetentionPolicy;
}

export interface AuditRetentionPolicy {
  policy: "keep_all" | "days";
  /** Required when `policy === "days"`. Events older than this window are purged. */
  days?: number;
}
...
export interface AppMetaConfig {
  ...
  emoji?: string;
  /** Optional folder id (references AdminConfig.folders with kind="forms"). */
  folderId?: string;
