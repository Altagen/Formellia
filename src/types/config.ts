...
export interface AdminPage {
  ...
  /**
   * Set by the auto-page generator to the source form's id. Pages that carry
   * this field were created by the platform and are removed when the form is
   * deleted — manually created pages should never set it.
   */
...
export interface AdminFeatures {
  ...
  auditLog?: boolean;
  /**
   * When a form is created (via API, YAML upsert, or restore), automatically
   * append a dashboard page that lists that form's submissions in a
   * `submissions_table` widget. The page is bound to the form's id and is
   * removed when the form is deleted. Default: false.
   *
   * Backfill is also available — turning the toggle on (or invoking the
...
export interface AdminConfig {
  ...
  /**
   * Predefined reasons offered in dropdowns when the operator excludes a
   * submission from a DataPool (or, in the future, performs other gated
   * actions). Empty/absent → free-text only. Operators can always type a
   * custom reason on top of these via the "Autre…" option in the dialog.
   *
   * Each team has its own vocabulary (e.g. an event organiser uses
   * "Demande de désinscription", a B2B uses "Bounce / RGPD Art.21", a
   * support team uses "Spam complaint / Duplicate request"). Surfacing
   * the list as config means it's reusable + auditable + tied to the
   * deployment's policy, not hardcoded.
   */
